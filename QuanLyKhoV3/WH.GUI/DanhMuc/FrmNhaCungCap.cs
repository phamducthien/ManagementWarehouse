...
namespace WH.GUI
{
    public partial class FrmNhaCungCap : FrmBase
    {
        ...
        public List<NHACUNGCAP> DataList { get; set; }
        ...
        private void ActionXoa()
        {
            if (!SetGuiToData()) return;
            if (Model == null) return;
            if (ShowMessage(IconMessageBox.Question, "Bạn có chắc muốn xóa ncc " + Model?.TENNHACUNGCAP + " không?") ==
        ...
        {
            Model = Service.Find(s => s.MANHACUNGCAP == Model.MANHACUNGCAP) ?? Service.CreateNew();
            txtTen.Text = Model.TENNHACUNGCAP;
            txtDiaChi.Text = Model.DIACHI;
            txtDiaThoai.Text = Model.DIENTHOAI;
            txtDiDong.Text = Model.DIDONG;
            txtDaiDien.Text = Model.NGUOIDAIDIEN;
        ...
        private bool SetGuiToData()
        {
            if (ThaoTac != ThaoTac.Xoa)
                ...
                {
                    ShowMessage(IconMessageBox.Warning, "Vui lòng điền thông tin 'Tên Nhà Cung Cấp'");
                    GetDataFromDgv();
                    return false;
                }

            if (Model != null)
                Model = Service.Find(s => s.MANHACUNGCAP == Model.MANHACUNGCAP);
            if (Model == null)
            {
            ...
            Model.DIENTHOAI = txtDiaThoai.Text;
            Model.EMAIL = txtEmail.Text;
            Model.NGUOIDAIDIEN = txtDaiDien.Text;
            return true;
        }
