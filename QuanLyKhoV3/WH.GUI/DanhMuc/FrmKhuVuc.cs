...
namespace WH.GUI
{
    public partial class FrmKhuVuc : FrmBase
    {
        public FrmKhuVuc()
        {
            InitializeComponent();
        ...
        public KHACHHANGKHUVUC Model { get; set; }
        public List<KHACHHANGKHUVUC> DataList { get; set; }

        private IKHACHHANGKHUVUCService Service
        {
            get
            {
                ReloadUnitOfWork();
            ...
            try
            {
                if (ThaoTac != ThaoTac.Them && ThaoTac != ThaoTac.Sua && IsSelect)
                {
                    ...
                    if (dgvDanhMuc.SelectedRows.Count > 0)
                    {
                        ...
                        Model = Service.Find(s => s.MAKHUVUC.ToString().Equals(sId));
                        CurrentRow = row;
                        SetDataToGui();
                    }
                }
        ...
        {
            Model = Service.Find(s => s.MAKHUVUC == Model.MAKHUVUC) ?? Service.CreateNew();
        ...
        private bool SetGuiToData()
        {
            if (ThaoTac != ThaoTac.Xoa)
                ...
                {
                    ShowMessage(IconMessageBox.Warning, "Vui lòng điền thông tin 'Tên Khu Vực'");
                    GetDataFromDgv();
                    return false;
                }

            if (Model != null)
                Model = Service.Find(s => s.MAKHUVUC == Model.MAKHUVUC);
            if (Model == null)
            {
