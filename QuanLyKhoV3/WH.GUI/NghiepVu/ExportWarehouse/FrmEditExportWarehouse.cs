    ...
    public partial class FrmEditExportWarehouse : FrmBase
    {
        ...
        private void ActionHuyMatHang()
        {
            try
            {
                ...
                var result = xuatKhoService.HuyMatHangTrongHoaDon(MaHoaDon, hoaDonNhapKhoChiTiets);
                if (result != MethodResult.Succeed)
                    ShowMessage(IconMessageBox.Information, xuatKhoService.ErrMsg);
                else
                    LoadHoaDon();
        ...
        private void ActionTimKiem()
        {
            try
            {
                ...
                var xuatKhoService = XuatKhoService;
                var lstMatHangs = xuatKhoService.SearchMatHangs(textSearch);
        ...
        private void ActionThanhToan()
        {
            ...
            try
            {
                ...
                var xuatKhoService = XuatKhoService;
                var result = xuatKhoService.ThanhToan(MaHoaDon, tienChi, giamGia, txtGhiChu.Text);
                if (result != MethodResult.Succeed)
                {
                    ShowMessage(IconMessageBox.Information, xuatKhoService.ErrMsg);
                }
            ...
            try
            {
                if (IsSelect)
                {
                    ...
                    if (dgvHoaDon.SelectedRows.Count > 0)
                    {
                        ...
                        if (sId == "") return;
                        var xuatKhoService = XuatKhoService;
                        ModelChiTiet = xuatKhoService.GetModelChiTiet(sId);
                        KhoMatHangModel = xuatKhoService.GetModelKhoMatHang(ModelChiTiet.MAMATHANG.ToString());
                        CurrentRow2 = row;
                    }
                }
            }
