...
{
    public partial class FrmLoaiMatHang : MetroForm
    {
        ...
        private void ActionSearchData()
        {
            ...
            ILOAIMATHANGService service = new LOAIMATHANGService(_baseForm.UnitOfWorkAsync);
            List<LOAIMATHANG> list = service.Search(textSearch);
            LoadData(list);
        }
        ...
        {
            txtTen.Text = _model.TENLOAIMATHANG;
            NumDoUuTien.Value = _model.STATUS.ToString().ToIntOrDefault();
            txtGhiChu.Text = _model.MOTA;
        }
        /// <summary>
        /// Set Object
        /// </summary>
        private bool SetGuiToData()
        {
            ...
            _model.TENLOAIMATHANG = txtTen.Text;
            _model.MOTA = txtGhiChu.Text;
            _model.STATUS = NumDoUuTien.Value.ToString("####").ToIntOrDefault();
        ...
        private void SetTextReadOnly(bool isReadOnly)
        {
            ...
            foreach (var obj in gbxInfo.Panel.Controls.OfType<Control>().Where(obj => !(obj is KryptonTextBox) && !(obj is KryptonLabel)))
            {
        ...
        private void LoadData(List<LOAIMATHANG> list)
        {
            try
            {
        ...
        #region Extends
        private void btnUpDoUuTien_Click(object sender, EventArgs e)
        {
            NumDoUuTien.UpButton();
            NumDoUuTien.Select(NumDoUuTien.ToString().Length, 0);
        }

        private void btnDownDoUuTien_Click(object sender, EventArgs e)
        {
            NumDoUuTien.DownButton();
            NumDoUuTien.Select(NumDoUuTien.ToString().Length, 0);
        }
        #endregion
    }
